...
export class UserController {
  ...
  async createRecognition(
    ...
    return this.recognitionMapper.map(
      await this.userService.createRecognition(file, userData),
    );
  }
  ...
  async findById(
    ...
    return await this.userMapper.map(
      await this.userService.findById(userData.id),
    );
  }
  ...
  async findHistoryById(
    ...
    return this.recognitionMapper.mapAll(
      await this.userService.findHistory(userData.id),
    );
  }
